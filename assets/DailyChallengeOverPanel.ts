...
export default class DailyChallengeOverPanel extends Component {
    ...
    private buildWinUI(card: Node): void {
        ...
        const contBtn = createRoundedButton('continueBtn', btnArea, '继续闯关', 440, 62, COLOR_TEAL, COLOR_WHITE, 22, 22);
        contBtn.node.setPosition(0, 18, 0);

        const shareBtn = createRoundedButton('shareBtn', btnArea, '分享好友', 440, 56, COLOR_AD_BLUE, COLOR_WHITE, 20, 20);
    ...
    private createTitleArea(card: Node, isWin: boolean, title: string, subtitle: string): Node {
        ...
        const titleLabel = createLabel('title', titleArea, title, 38, isWin ? COLOR_GOLD : COLOR_RED, true);
