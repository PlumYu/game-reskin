...
export default class SettingPanel extends Component {
    ...
    private buildUI(): void {
        ...
        // 标题
        const title = createLabel('title', card, '猫窝设置', 32, SETTING_TEXT, true);
    ...
    private showSurvivalExitConfirm(): void {
        ...
        const question = createLabel('SurvivalExitQuestion', messageBox, '确定先离开这局找猫吗？', 28, SETTING_TEXT, true);
        question.node.getComponent(UITransform)!.setContentSize(new Size(messageW - 50, 44));
        question.node.setPosition(0, 26, 0);

        const warning = createLabel('SurvivalExitWarning', messageBox, '主动退出会直接结算本次找猫挑战！', 25, new Color(214, 100, 73, 255), true);
