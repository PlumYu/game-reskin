...
export function installDrawGridToolbar(target: any): void {
    ...
    buildToolbar: function (): void {
        ...
        const tools: { name: string; cost: number; size: number; iconSize: number; handler: () => void; drawIcon: (g: Graphics) => void }[] = [
            ...
            { name: '占座', cost: COW_HINT_COIN_COST, size: cardSize, iconSize, handler: this.onToolReveal.bind(this), drawIcon: this.drawToolCowIcon },
    ...
    useHint: function () {
        ...
        if (hiddenCowIndices.length <= 0) {
            ...
            this.showWorkplaceToast('所有占座目标都已经找齐了', v3(0, 330, 0), new Color(180, 190, 205, 255));
            return;
        }
        ...
        this.showWorkplaceToast('占座定位器，直接锁一格', v3(0, 330, 0), new Color(255, 238, 92, 255));
