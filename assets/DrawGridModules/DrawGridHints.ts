...
export function installDrawGridHints(target: any): void {
    ...
    joinHintColorCellNames: function (colors: string[]): string {
        ...
        return uniqueCount > 1 ? '这几种样式的格子' : '这种口味的格子';
    },
    showHint: function () {
    ...
    showWrongExcludeHint: function (): boolean {
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (label) {
                label.string = '这个格子被你排除了，但它其实就是奶茶的位置；点一键应用，把它找出来。';
            }
    ...
    showLogicHint: function (): boolean {
        ...
        const label = this.hintPanelNode.getComponentInChildren(Label);
        if (label) {
            label.string = `${reasonColorCellName || '这种口味的格子'}只剩这一个还可能藏着奶茶，所以奶茶就在这里。`;
        }
        this.setHintPanelShown(true);
        this.renderGrid();
        return true;
    ...
    showExcludeRowColHint: function (): boolean {
        ...
        for (let cowIdx = 0; cowIdx < total; cowIdx++) {
            ...
            const label = this.hintPanelNode.getComponentInChildren(Label);
            if (label) {
                label.string = '这行和这列已经锁定奶茶了；每行每列只能有1杯，所以同排同列其他格子都可以排除。';
            }
            this.setHintPanelShown(true);
            this.renderGrid();
            return true;
    ...
    showNeighborExcludeHint: function (): boolean {
        ...
        for (let cowIdx = 0; cowIdx < total; cowIdx++) {
            ...
            const label = this.hintPanelNode.getComponentInChildren(Label);
            if (label) {
                label.string = '奶茶之间不能相邻；这杯奶茶周围一圈都不可能再有奶茶，可以排除。';
            }
            this.setHintPanelShown(true);
            this.renderGrid();
            return true;
    ...
    showOccupiedRowsHint: function (): boolean {
        ...
        for (let mask = 1; mask < (1 << n); mask++) {
            ...
            const label = this.hintPanelNode.getComponentInChildren(Label);
            if (label) {
                label.string = `剩下的${colorCellNames}只能分布在这${rows.length}行里；这些行里的其他格子不会藏奶茶，可以排除。`;
            }
            this.setHintPanelShown(true);
            this.renderGrid();
            return true;
        ...
        for (let mask = 1; mask < (1 << n); mask++) {
            ...
            const label = this.hintPanelNode.getComponentInChildren(Label);
            if (label) {
                label.string = `剩下的${colorCellNames}只能分布在这${cols.length}列里；这些列里的其他格子不会藏奶茶，可以排除。`;
            }
            this.setHintPanelShown(true);
            this.renderGrid();
            return true;
    ...
    showNeighborColorOnlyHint: function (): boolean {
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (label) {
                ...
                label.string = `如果这个格子藏着奶茶，旁边的${colorCellName}就没有合法位置了；所以这个格子可以排除。`;
            }
            this.setHintPanelShown(true);
            this.renderGrid();
            return true;
