...
- 体力值显示框宽度由 `HUD_STAMINA_PILL_WIDTH` 控制，文字区域由 `staminaTextGroupWidth` 控制。
  ...
  - 当前已调整为框宽 252 + 文字区 168，确保三位数不溢出。
  - 换皮时如果更换了 `hud_stamina_frame.png` 图片，需要确认新图片尺寸与代码宽度匹配。

### 21. 本地小图不能只换 PNG

- `assets/images/` 下面这批小图经常直接挂在场景节点上，除了 `png` 之外，`*.meta` 里的裁切信息也会直接影响显示范围：
  - `tips1.png`
  - `tips2.png`
  - `clear.png`
  - `point.png`
...
- 如果只覆盖图片、不更新 `trimX / trimY / width / height / offsetX / offsetY`，Cocos 仍然会沿用旧的透明边界，结果就是图标被裁歪、按钮点位对不上，或者提示框里的图看起来像“缩进去一块”。
- 处理方式：
  - 已有资源优先保留原 `.meta` 的 uuid，只更新裁切数据。
  - 新增资源（例如远程提示图 `提示-找牛` / `提示-排除`）再单独生成新的 `.meta`。
  - 换完后至少抽查一次 `scene.scene` 里直接引用的节点，确认不是“图片换了，但显示框还是旧的”。


### 22. 首页入口和结算弹层不能只做“资源已替换”验收

- `assets/MainPanel.ts` 里的每日挑战、生存模式、排行榜入口都是“底图 + 代码字 + 代码布局”的组合；底图换完以后，必须再验一次：
  - 文案是否压在浅底或高光上看不清
  - 倒计时、免费次数、提示字是否仍在安全区域内
  - 入口卡片是否需要重新排版，而不是继续沿用原皮布局
- 首页入口文案不只是“看得清”就够，文字还必须落在底图真正的留白区里；如果坐标还是沿用原皮左偏布局，就会出现“字没压色，但压错位置”的问题。
- 不要用大块白底/白板去硬兜首页入口可读性；它会破坏底图本身的主题质感。正确做法是优先重排文字锚点、对齐方式、宽度和描边，让文案自己回到留白区。
- 这一轮猫主题里，首页最终保留下来的有效做法有三件事：
  - 两张入口卡片错位摆放并微调角度，避免还是原版那种机械平铺
  - 排行榜入口补独立文字承载层，不能只指望底图自带留白
  - 每日挑战、生存模式的文案改成按留白中心重新排版，而不是继续沿用原皮左偏坐标
- `assets/SettingPanel.ts`、`assets/FailPanel.ts`、`assets/DailyChallengeOverPanel.ts`、`assets/SurvivalOverPanel.ts` 都是代码绘制面板；换图不会自动把这些页面换成新主题。
- 结算/弹窗类页面的最小验收清单要固定下来：
  - 首页设置弹框
  - 生存模式退出确认
  - 普通闯关失败
  - 每日挑战失败
  - 生存失败
- 清单里“已记录”不等于“已完成”。这类页面必须在微信开发者工具里逐页触发一遍，再回头勾掉，不然最容易出现“README 写了，但实际没改”的假完成。
